...
- Fonctions : toujours utiliser des accolades `{}` pour le corps, même pour une seule expression. Pas de fonctions fléchées inline sans accolades (ex: `const foo = (x) => { return x + 1 }`, pas `const foo = (x) => x + 1`). **Exception** : les callbacks d'une ligne à l'intérieur d'un `pipe` peuvent omettre les accolades et le `return` (ex: `Array.filter((x) => x > 0)`, `Effect.map(Boolean.not)`). Les fonctions nommées et les corps multi-lignes gardent toujours les accolades.
...
- **Frontières SQL** : valider les données avec `Schema.decode` (lecture) et `Schema.encode` (écriture). Utiliser `Schema.decodeUnknown` uniquement quand les types de la source ne sont pas connus (données externes non typées). Toujours extraire le décodeur/encodeur dans une `const` nommée au niveau module (ex : `const decodeFoos = Schema.decode(Schema.Array(Foo))`) — jamais de double appel inline `Schema.decode(...)(data)`.
...
- **Ensembles** : utiliser `HashSet` de Effect plutôt que `Set` natif. Créer via `HashSet.fromIterable(...)`, tester via `HashSet.has(set, value)`, itérer via `HashSet.forEach`.
- **Dictionnaires** : utiliser `HashMap` de Effect plutôt que `Map` natif. Créer via `HashMap.fromIterable(...)`, accéder via `HashMap.get(map, key)` (retourne `Option`), itérer via `HashMap.toEntries`.
- **Accesseurs de propriétés** : utiliser `Struct.get("prop")` plutôt que `(x) => { return x.prop }` pour les callbacks d'accès simple à une propriété (ex : `Array.map(items, Struct.get("id"))`).
- **Pipelines de transformation** : privilégier `pipe(data, Array.map(...), Array.filter(...), ...)` plutôt que d'imbriquer les appels ou d'utiliser des variables intermédiaires. Le style `pipe` rend le flux de données linéaire et lisible. Utiliser aussi `pipe` avec les `Effect` : `pipe(Effect.forEach(...), Effect.map(...))` plutôt que `Effect.map(Effect.forEach(...), ...)`. Exception : dans les `Effect.gen` et `Effect.fnUntraced`, utiliser `yield*` directement.
- **Comparateurs data-last** : les fonctions de comparaison Effect (`DateTime.lessThan`, `DateTime.greaterThan`, `Array.contains`, etc.) en forme curryfiée sont data-last : `DateTime.greaterThan(dt)(self)` = `self > dt`. Attention au sens lors du piping : `pipe(DateTime.now, Effect.map(DateTime.greaterThan(dt)))` = `now > dt`.
