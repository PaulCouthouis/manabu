...
- **Pipelines de transformation** : privilégier `pipe(data, Array.map(...), Array.filter(...), ...)` plutôt que d'imbriquer les appels `new Set(Array.map(...))`. Le style `pipe` rend le flux de données linéaire et lisible.
