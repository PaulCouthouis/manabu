...
describe("kana-extended-data", () => {
  ...
  it("contains 3 sokuon/chōon", () => {
    const sokuonChoon = Array.filter(kanaExtendedData, (k) => sokuonChoonIds.has(k.id))
    expect(sokuonChoon.length).toBe(3)
  })

  it("contains 13 extended katakana", () => {
    const extended = Array.filter(kanaExtendedData, (k) => !sokuonChoonIds.has(k.id))
    expect(extended.length).toBe(13)
  })

  // AC-K2 — IDs continus 209-224
  it("has continuous IDs from 209 to 224", () => {
    const ids = Array.map(kanaExtendedData, (k) => k.sortOrder)
  ...
  it("has no duplicate characters", () => {
    const characters = Array.map(kanaExtendedData, (k) => k.character)
