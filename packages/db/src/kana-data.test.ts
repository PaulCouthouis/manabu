...
describe("kana-data", () => {
  ...
  it("has continuous sortOrder from 1 to 208", () => {
    const sortOrders = Array.map(kanaData, (k) => k.sortOrder)
  ...
  it("has no duplicate characters", () => {
    const characters = Array.map(kanaData, (k) => k.character)
  ...
  it("each kana has a unique id matching its sortOrder", () => {
    const ids = Array.map(kanaData, (k) => k.id)
